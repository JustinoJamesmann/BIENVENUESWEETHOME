...
export function loadUsers(): User[] {
  ...
  const migrated = users.map((u: User) => {
    ...
    if (u.username === "worker") return { ...u, username: "BHS!" };
    return u;
  });
...
export function login(username: string, password: string): User | null {
  ...
  if (user && ((username === "BienvenueSweetHome" && password === "Bi!Swe!Ho123") || (username === "BHS!" && password === "BSH!0123"))) {
